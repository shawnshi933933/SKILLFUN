...
import { Zap, Wallet, ChevronDown, Menu, X } from "lucide-react";
...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { href: "/app/create", label: "+ Create Skill" },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.filter(l => l.href !== "/app/create").map((link) => (
            ...
            {address ? (
              <div className="relative">
                ...
                >
                  ...
                  <Badge variant="outline" className="text-xs border-accent/30 text-accent hidden sm:inline-flex">Sepolia</Badge>
                ...
                <div
                  className={`flex items-center px-4 py-3 rounded-xl text-sm font-medium transition-colors cursor-pointer ${
                    isActive(link.href)
                      ? "bg-primary/20 text-primary"
                      : "text-muted-foreground hover:text-foreground hover:bg-white/5"
                  } ${link.href === "/app/create" ? "border border-primary/20 text-primary mt-2" : ""}`}
